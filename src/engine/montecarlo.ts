...
export interface RunOutput {
  ...
  /** Present only when RunOptions.captureInputs was true. inputsSamples[i] is trial i's full sampled-driver-inputs record. */
  inputsSamples?: Record<string, number>[];
}

/** Draws one full input set for a decision (every driver + its discount rate). */
function sampleInputs(decision: Decision, rng: () => number): { inputs: Record<string, number>; discountRate: number } {
 ...
 * iteration against one shared macroFactor draw.
 */
export function runTrial(
  decision: Decision,
  rng: () => number,
  macroFactor = 0,
): { npv: number; cashFlows: number[]; inputs: Record<string, number> } {
  const { inputs, discountRate } = sampleInputs(decision, rng);
  const cashFlows = decision.cashFlows(guardInputs(inputs, decision.id), macroFactor);
  return { npv: npv(discountRate, cashFlows), cashFlows, inputs };
}
...
export function runBaseCase(decision: Decision): { npv: number; payback: number | null; cashFlows: number[] } {
  ...
  const discountRate = baseCase(decision.discountRate.distribution);
  const cashFlows = decision.cashFlows(guardInputs(inputs, decision.id));
  return { npv: npv(discountRate, cashFlows), payback: paybackPeriod(cashFlows), cashFlows };
}
